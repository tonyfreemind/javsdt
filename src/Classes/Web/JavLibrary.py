...
class JavLibrary(object):
    ...
    def get_library_html(self, url):
        for retry in range(10):
            try:
                if self.proxies:
                    rqs = requests.get(url, proxies=self.proxies, timeout=(6, 7))
                else:
    ...
    def scrape(self, jav_file: JavFile, jav_model: JavData):
        ...
        if '图书馆' in jav_file.Name:
            ...
            if url_appointg:
                url_jav_library = f'{self.url}/?v={url_appointg.group(1)}'
                print(f'    >指定网址: {url_jav_library}')
                html_jav_library = self.get_library_html(url_jav_library)
        ...
        # 用户没有指定网址，则去搜索
            ...
            url_search = f'{self.url}/vl_searchbyid.php?keyword={jav_file.Car}'
            print(f'    >搜索javlibrary: {url_search}')
            # 得到javlibrary搜索网页html
            html_search_library = self.get_library_html(url_search)
            ...
            # 第二种情况: 搜索结果可能是两个以上，所以这种匹配找不到标题，None！
                ...
                if list_search_results:
                    # 默认用第一个搜索结果
                    url_jav = f'{self.url}/?v=jav{list_search_results[0][0]}'
                    ...
                    if len(list_search_results) > 1 and not list_search_results[1][1].endswith('ク）'):  # ク）是蓝光重置版
                        ...
                        if list_search_results[0][1].endswith('ク）'):
                            url_jav = f'{self.url}/?v=jav{list_search_results[1][0]}'
